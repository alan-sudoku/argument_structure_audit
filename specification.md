...
- *Repair (application scope):* prioritise the sentence pass on blocks that marginally passed the block pass; do not apply globally to every sentence in every block that passed without being marginal, unless sentence-level precision is required by the compliance target (see §0).
...
1. **`CONTENT-TYPE` · T1** — separate content types. Moving Arguments and Closures out of Claims is the fix with the largest downstream effect; it reduces the wall before restructuring it.
