...
- *Pass:* Every sub-item label identifies content type using `*[Type] ([Topic]):*` format. Types: Claim, Scope, Argument, Closure. Topic is a 1–4 word phrase naming the subject of the item, extracted from the item's lead sentence (e.g., `*Argument (cost implications):*`, `*Scope (quantifier range):*`). This is a syntactic check — it verifies label format only, not whether the label's type accurately describes the item's content. Semantic label accuracy (wrong type assigned to an item) requires T2 review.
